...
const DoctorDashboard = () => {
  const { userAuth: { access_token, fullname, role, isAdmin } } = useContext(UserContext);
  const [appointments, setAppointments] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
      ...
      <div>
        ...
        <h5 style={{ textTransform: 'capitalize', textAlign: 'center', fontWeight: 'bold', color: 'red', borderBottom: '2px solid black', paddingBottom: '1rem' }}>
          {role} dashboard
            ...
            {
              isAdmin ? <p style={{fontWeight:'bolder'}}>ADMIN</p> : <p style={{fontWeight:'bolder'}}>NOT ADMIN</p>
            }
        ...
        <div className="col-12 mb-3" style={{ textAlign: 'center' }}>
          ...
          {!loading && !error && (
            <>
              ...
              <h4 style={{ marginTop: '2rem', marginBottom: '2rem', paddingBottom: '1rem' }}>Patient's Appointments</h4>
              {appointments.length > 0 ? (
                <ul>
                  {appointments.map((appointment, index) => (
                    <div key={appointment._id} className={`appointment-item ${index % 2 === 0 ? 'even' : 'odd'}`}>
